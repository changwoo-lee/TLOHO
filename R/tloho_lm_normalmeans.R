#' TLoHolm: Tree-based Low rank Horseshoe regularization prior in linear model
#' 
#' 
#'
#' Anonymous authors, 
...
#' 
#' 
tloho_lm_normalmeans <- function(Y, graph0, init_val=NULL, c = 0.5, tau0 = 1, MCMC = 50000, BURNIN = 40000, THIN = 10, Dahl = F, seed=NULL){
  ## sanity check ----
  set.seed(seed)
  n = length(Y)
  p = n
  ...
  XtX = diag(1, nrow = k) #crossprod(Xtilde) # this is always I_k
  diag(XtX) = diag(XtX) + PRECISION # initial local parameter is 1
  R = diag(sqrt(1+PRECISION), nrow = k) #chol(XtX) # this is always diag(rep(sqrt(1+PRECISION), k))
  
  sigmasq_y = 1 # noise variance initialize
  # t(Xtilde) %*% Y
  ...
  for(iter in 1:MCMC) {
      ...
      if(move == 1) { 
        ...
        # split step: proposing two cluster
        proposedlogL = logHSmarginal(Y[vid_old], sigma2 = sigmasq_y, tau2 = tau2) +
          logHSmarginal(Y[vid_new], sigma2 = sigmasq_y, tau2 = tau2)
        # original: one cluster
        originallogL = logHSmarginal(Y[c(vid_old,vid_new)], sigma2 = sigmasq_y, tau2 = tau2)
        
        log_L = proposedlogL - originallogL
        ...
        if(runif(1) < acc_prob){
          ...
          # no need to update Xtilde, we don't use it in likelihood calculation
          ...
          #R = R_new; Xty = Xty_new
          lambda2 = append(lambda2, 1, after = clust_new-1)
          PRECISION = 1/(lambda2*tau2)
          lambda2 = 1/(PRECISION*tau2)
          edge_status = update_res$estatus
          
          R <- diag(sqrt(1+PRECISION), nrow = k)
          Xty = getPhi_y(clust_vid, p, y = Y)
          log_like_res = evalmLogLike_lambdavec(R, Xty, PRECISION, a0, b0, yty, n)
          log_like = log_like_res$mlog_like
      ...
      if(move == 2) { 
        ...
        # merge step: proposing one cluster
        proposedlogL = logHSmarginal(Y[c(vid_old,vid_new)], sigma2 = sigmasq_y, tau2 = tau2)
        # original: two cluster
        originallogL = logHSmarginal(Y[vid_old], sigma2 = sigmasq_y, tau2 = tau2) +
          logHSmarginal(Y[vid_new], sigma2 = sigmasq_y, tau2 = tau2)
      ...
      if(move == 3){ 
        ...
        originallogL = logHSmarginal(Y[vid_old], sigma2 = sigmasq_y, tau2 = tau2) +
          logHSmarginal(Y[vid_new], sigma2 = sigmasq_y, tau2 = tau2)
        diff1 = proposedlogL - originallogL
        # then perform split move
        ...
        # original: one cluster
        originallogL = logHSmarginal(Y[c(vid_old,vid_new)], sigma2 = sigmasq_y, tau2 = tau2)
        diff2 = proposedlogL - originallogL
      ...
      R_new <- diag(sqrt(1+PRECISION_newtau), nrow = k)
    ...
    if(iter > BURNIN & (iter - BURNIN) %% THIN == 0) {
      ...
      Phi = getPhi(clust_vid, p)
      beta_est[idx, ] = crossprod(Phi, beta_tilde)
      ...
      log_post_out[idx] = evalLogPost_HS(beta_tilde, sigmasq_y, lambda2, tau2, k, Y, t(Phi), hyper, Comp)
    }
