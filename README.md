...
fit_hsplus <- tloho_lm(data$Y, data$X, graph0, hsplus =T) # added in ver 1.1.0 (experimental)
...
# black (hsplus) is more highly peaked at 0


node = 500 # node that true beta = -1
plot(density(fit_hsplus$beta_out[,node]), main = "posterior of beta, black:hsplus, red:hs ")
lines(density(fit$beta_out[,node]), col = "red")

node = 900 # node that true beta = 0
plot(density(fit_hsplus$beta_out[,node]), main = "posterior of beta, black:hsplus, red:hs ")
lines(density(fit$beta_out[,node]), col = "red")
```
